# ============================================================
# Final literature-nominal program-feedback case
#
# Fixed conditions
# ----------------
...
#
# The corresponding volume density is calculated using:
#
...
# Important
# ---------
# The dictionary returned to run_program_feedback_compare.py
# intentionally contains only fields supported by its original
# CSV field list.
#
# BlockingOxide thickness is added later as external metadata.
# ============================================================
...
EXISTING_FEEDBACK_SCRIPT = (
    SCRIPT_DIRECTORY
    / "run_program_feedback_compare.py"
)

EXISTING_FIELD_SWEEP_SCRIPT = (
    SCRIPT_DIRECTORY
    / "run_positive_field_sweep.py"
)
...
RESULTS_DIRECTORY = (
    SCRIPT_DIRECTORY
    / "results"
    / "final_program_literature_nominal_3_5_16nm_16V"
)
...
def validate_settings():
    if not EXISTING_FEEDBACK_SCRIPT.exists():
        raise FileNotFoundError(
            "run_program_feedback_compare.py was not found:\n"
            f"{EXISTING_FEEDBACK_SCRIPT.resolve()}"
        )

    if not EXISTING_FIELD_SWEEP_SCRIPT.exists():
        raise FileNotFoundError(
            "run_positive_field_sweep.py was not found:\n"
            f"{EXISTING_FIELD_SWEEP_SCRIPT.resolve()}"
        )
...
def thickness_to_token(thickness_nm):
    return (
...
def build_case_name(
    ...
    token = thickness_to_token(
        charge_trap_thickness_nm
    )

    return f"charge_trap_{token}nm"
...
# ============================================================
# Extend the original static-point CSV schema
#
# The configured static-point function records the selected
# BlockingOxide thickness. The original feedback script did not
# contain this column, so its CSV field list must be extended.
# ============================================================

if hasattr(
    feedback,
    "STATIC_POINTS_FIELDNAMES",
):
    original_static_fieldnames = tuple(
        feedback.STATIC_POINTS_FIELDNAMES
    )

    if (
        "blocking_oxide_thickness_nm"
        not in original_static_fieldnames
    ):
        feedback.STATIC_POINTS_FIELDNAMES = (
            original_static_fieldnames
            + (
                "blocking_oxide_thickness_nm",
            )
        )
...
# ============================================================
# Override original feedback-script settings
# ============================================================
...
# ============================================================
# Correct density conversion for each ChargeTrap thickness
# ============================================================
...
# ============================================================
# Replacement static feedback-point calculation
# ============================================================
...
def configured_run_static_feedback_point(
    ...
    # IMPORTANT
    #
    # Do not add blocking_oxide_thickness_nm to this dictionary.
    #
    # run_program_feedback_compare.py writes this result using
    # its original STATIC_POINTS_FIELDNAMES. That original field
    # list does not contain blocking_oxide_thickness_nm.
    #
    # BlockingOxide thickness is added later by the outer script.
    # ========================================================
...
# ============================================================
# Combine detailed case CSV files
# ============================================================
...
def write_combined_comparison_csv(rows):
...
def print_summary(summary_rows):
    print_section(
        "FINAL PROGRAM CASE FEEDBACK SUMMARY"
    )
...
def main():
    ...
    print_section(
        "FINAL LITERATURE-NOMINAL PROGRAM CASE COMPLETED"
    )
    ...
    print(
        "1. This run evaluates the fixed 3/5/16 nm structure "
        "at Program VG=16 V."
    )

    print(
        "2. The target volume density is obtained from the "
        "target sheet density and the 5 nm ChargeTrap thickness."
    )

    print(
        "3. The feedback time includes the reduction of oxide "
        "field and FN current as trapped charge accumulates."
    )

    print(
        "4. The result remains an FN-only, unit-capture-efficiency "
        "model estimate; retention and de-trapping are not included."
    )
...
if __name__ == "__main__":
    print(
        "RUN_FINAL_PROGRAM_CASE SCRIPT STARTED"
    )
